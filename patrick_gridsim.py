...
class IceCreamGridworld:
    """
    Canonical gridworld with 5 actions and transition noise.
    Invalid destinations (off grid or obstacle) cause a stay.
    Observation is a probabilistic rounding of the harmonic-mean distance to RD and RS.
    """
    ...
    def transition_probs(self, s: Tuple[int, int], a: Action) -> Dict[Tuple[int, int], float]:
        """
        P(s'|s,a): chosen action succeeds with prob 1 - p_error,
        otherwise one of the 4 alternates happens uniformly.
        """
...
def reward_terminal_once(env, task: TaskSpec, s_next: State) -> Tuple[float, bool]:
    t = is_store(env, s_next)
    if t == "RD":
        return task.RD, True
    if t == "RS":
            ...
            for a in ACTIONS:
                ...
                for s_next, p in probs.items():
                    if task.reward_mode == "start_state":
                        ...
                        r, done = reward_terminal_once(env, task, s_next)
                    j = idx[s_next]
